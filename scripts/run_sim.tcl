...
proc run_simulation_library { } {
    ...
    foreach test $test_dict {
        ...
        puts "INFO : Testing library $test_name with sources $source_names and testbenches $testbench_names"

        foreach source_name $source_names {
            puts "Compiling source $source_name"
            set source_split [split $source_name "."]
            set source_ext [lindex $source_split end]
            if { $source_ext == "vhd" } {
                exec xvhdl -nolog "$script_dict/../../libraries/$source_name"
            } else {
                exec xvlog -nolog "$script_dict/../../libraries/$source_name"
            }
        }

        foreach tb $testbench_names {
            set tb_split [split $tb "."]
            set tb_ext [lindex $tb_split end]
            if { $tb_ext == "vhd" } {
                exec xvhdl -nolog "$script_dict/../../test_libraries/$tb"
            } else {
                exec xvlog -nolog "$script_dict/../../test_libraries/$tb"
            }
        }
        ...
        *
        ...
        set last_tb_split [split $last_tb "."]
        set last_tb_name [lindex $last_tb_split 0]
        exec xelab -nolog -relax -debug typical $last_tb_name -s top_sim
        xsim top_sim -nolog
        run -all
    }
}
...
proc run_simulation_project {test_dict} {
    ...
    foreach test $test_dict {
        ...
        # Compile sources and testbench (VHDL-2008)
        puts "INFO : Testing project test $test_name with sources $source_names and testbenches $testbench_names"

        foreach source_name $source_names {
            set source_split [split $source_name "."]
            set source_ext [lindex $source_split end]
            if { $source_ext == "vhd" } {
                exec xvhdl -nolog "$script_dict/$source_name"
            } else {
                exec xvlog -nolog "$script_dict/$source_name"
            }
        }
        ...
        foreach tb $testbench_names {
            ...
            set tb_ext [lindex $tb_split end]
            if { $tb_ext == "vhd" } {
                exec xvhdl -nolog "$script_dict/testbench/$tb"
            } else {
                exec xvlog -nolog "$script_dict/testbench/$tb"
            }
        }
